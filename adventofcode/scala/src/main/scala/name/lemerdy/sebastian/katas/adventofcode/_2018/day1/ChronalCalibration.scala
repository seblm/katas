...
object ChronalCalibration {
  ...
  @tailrec
  def firstFrequencyReachedTwice(
      frequencyChanges: Iterator[BigInt]
  )(currentFrequencyChanges: Seq[BigInt], sum: BigInt = 0, frequenciesReached: Set[BigInt] = Set.empty): BigInt =
    currentFrequencyChanges match {
      case Nil =>
        firstFrequencyReachedTwice(frequencyChanges)(frequencyChanges.duplicate._2.toSeq, sum, frequenciesReached)
