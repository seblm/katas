...
object InventoryManagementSystem {
  ...
  @tailrec
  private def countLetters(boxIDs: Seq[String],
                           twoLetters: Int = 0,
                           threeLetters: Int = 0): (Int, Int) = boxIDs match {
    case Nil => (twoLetters, threeLetters)
    case boxID :: tail => countLetters(
      boxIDs = tail,
  ...
  private def commonLettersBetweenTwoCorrectBoxIDs(boxIDs: List[String]): String = boxIDs match {
    case Nil => ""
    case boxID :: tail => tail.flatMap(sameLettersIfOnlyOneLetterIsDifferent(boxID))
      .headOption
